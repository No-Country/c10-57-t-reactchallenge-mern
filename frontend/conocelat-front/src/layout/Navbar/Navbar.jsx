...
export const Navbar = () => {
    ...
    useEffect(()=>{
        fetch("http://localhost:5173/users").then((resp)=>{
            return resp.json();
        }).then((resp)=>{
        ...
        <nav className="data-container navbar-container shadow w-full top-0 left-0">
            <div className="data__container-option  md:flex items-center justify-between  px-4 py-4 md:px-8">
                ...
                <div className="data__container-buttom">
                    ...
                    <button className="butoom-login" style={{ display: userExists === "" ? "block" : "none"}} onClick={handleLogin}>Login</button>
                    <div className="relative" style={{ display: userExists === "" ? "none" : "block"}}>
                        <button
